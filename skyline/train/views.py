...
def Add_Trai(request):
    ...
    if request.method == "POST":
        ...
        en=Add_Train(trainname=n,train_no=no,from_city=f,to_city=to,departuretime=de,arrivaltime=a,trevaltime=t,distance=d)
        en.save()
        error=True
    d={"error":error}
    return render(request,'addtrain.html',d)

#TRAIN LIST VIEWS
def viewtrai(request):
    if not request.user.is_authenticated:
        return redirect('login')
...
def Edit_route(request,pid):
    ...
    dm={"data":data,"data2":data2,"error":error}
    return render(request,'editroute.html',dm)
...
def Register_customer(request):
    ...
    if request.method == "POST":
        ...
        Register.objects.create(user=user,add=a,mobile=m,dob=d)
        error = True
    d = {'error':error}
...
def Search_Train(request):
    ...
    if request.method=="POST":
        ...
        if fare3<5 and fare3>0:
            ...
            fare3 = fare3
        route = f+" to "+t
        St.objects.create(fare=fare3,train_name="bus2",date3=da)
        for i in ase:
            coun = coun + 1
            error=True

    d={"data2":data,'route1':route1,'fare3':fare3,"error":error,'coun':coun,'route':route}
    return render(request,'searchtrain.html',d)
...
def Delete_passenger(request,pid,bid,route1):
    ...
    return redirect('bookdetail', coun,bid,route1)
...
def deletebooking(request,pid):
    ...
    error=True
    d = {'error':error}
    return render(request,'viewbooking.html',d)
...
def Card_Detail(request,total,coun,route1,pid):
    ...
    return render(request,'carddetail.html',d)
...
def view_ticket(request,pid):
    ...
    d = {'book':book}
    return render(request,'viewticket.html',d)